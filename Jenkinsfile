pipeline {
    ...
    stages {
        ...
        */
        ...
        stage('Versioning artifact'){
            steps{
                sh '''
                      mkdir -p versions
                      cp target/mvc_1.war versions/mvc_1:v$BUILD_ID.war
       ...
       stage('Deploying artifacts to tomcat'){
            steps{
               deploy adapters: [tomcat9(credentialsId: 'tomcat_newcred', path: '', url: 'http://54.191.185.160:8080/')], contextPath: null, onFailure: false, war: '**/*.war'
